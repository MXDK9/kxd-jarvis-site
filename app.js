...
const S = { thinking: false, synth: window.speechSynthesis, voice: null, history: [{ role: "system", content: "You are KD (also known as KXD AI), an advanced personal AI assistant. If the user asks you to create, draw, or generate an image, reply ONLY with this exact format: [IMAGE: a highly detailed description of the image]. Do not add any other text around it. Otherwise, answer questions normally. Keep answers concise. Refer to the user as 'Boss'." }] };
function init() { loadVoices(); addMsg('assistant', "Neural link established. KXD AI is online. Visual Image Generation Active.", true, false); }
function loadVoices() { const vs = S.synth.getVoices(); S.voice = vs.find(v => v.name.includes('Google US English') || v.name.includes('Samantha') || v.name.includes('Daniel')) || vs[0]; }
...
function addMsg(role, text, speakIt = false, push = true) {
    ...
    if (role === 'assistant' && text.includes("[IMAGE:")) {
        ...
        if (promptMatch && promptMatch[1]) {
            ...
            S.history.push({ role: "assistant", content: "Generated an image of: " + imagePrompt }); return;
        }
    }
    const div = document.createElement("div"); div.className = "msg " + role;
    div.innerHTML = '<div class="msg-inner">' + text + '</div>'; box.appendChild(div); box.scrollTop = box.scrollHeight;
...
async function processInput(input) {
    ...
    addMsg('user', input, false, true); S.thinking = true;
    const status = document.getElementById('status-dot'); if (status) status.className = 'status-dot thinking';
    const resp = await callAI(input); addMsg('assistant', resp, true, true);
    S.thinking = false; if (status) status.className = 'status-dot online';
}
function speak(text) {
    if (!S.synth) return; if (S.synth.speaking) S.synth.cancel();
    const cleanText = text.replace(/[*_#\[\]]/g, ''); const utt = new SpeechSynthesisUtterance(cleanText);
    utt.voice = S.voice; utt.pitch = 1.0; utt.rate = 1.1; S.synth.speak(utt);
}
document.getElementById('exec-btn').onclick = () => { const el = document.getElementById('txt'); if(el) { processInput(el.value); el.value = ''; } };
