...
window.login = async function() {
    ...
    if (window.ParticleEngine) {
        try {
            ...
            console.warn("Particle Engine error:", e);
        }
    }
    runBootSequence();
...
async function callOpenAI(userText) {
    ...
    const msgs = [
        ...
        ...S.history.slice(-20).map(h => ({ role: h.role === 'model' ? 'assistant' : 'user', content: h.parts[0].text })),
        { role: 'user', content: userText }
    ];
    try {
        const res = await fetch(CFG.OPENAI_URL, {
            method: 'POST',
...
async function init() {
    ...
    document.getElementById('send-btn')?.addEventListener('click', () => {
        ...
        if (t?.value.trim()) jarvisProcess(t.value.trim());
    });
